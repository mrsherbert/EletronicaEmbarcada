...
# 100 "main.c"
    while (1)
    {
        Vpwm =(uint16_t) (1.21) * ADC_GetConversion(1);


        setPWM();
        EPWM2_LoadDutyValue(pulse);
        if(ECCP1_IsCapturedDataReady()){
            TMR1_WriteTimer(0);
            Vcap = ECCP1_CaptureRead();
            PIR1bits.CCP1IF = 0;

            Const = 60 * 31250;

            Vrpm = (uint16_t) Const / Vcap;
        }
    }
}
