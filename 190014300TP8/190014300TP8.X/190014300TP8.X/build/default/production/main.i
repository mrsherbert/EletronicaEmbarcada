...
void TRUSART(){
    ...
    TX1dados.Tx1 = (uint16_t)((ADC_GetConversion(channel_AN0)* 1000 /1024) * vGAINt);



    TX1dados.vL1 = TX1dados.vL1 << 1;
    TX1dados.Tx1 = (TX1dados.Tx1 >> 1);
    TX1dados.vH1 = TX1dados.vH1 & 0x7F;



    TX2dados.Tx2 = (uint16_t)((ADC_GetConversion(channel_AN1)* 1000 /1024) * vGAINl);


    TX2dados.vL2 = TX2dados.vL2 << 1;
    TX2dados.Tx2 = (TX2dados.Tx2 >> 1);
    TX2dados.vH2 = TX2dados.vH2 & 0x7F;



    EUSART_Write(0x80);
    EUSART_Write(TX1dados.vH1);
    EUSART_Write(TX1dados.vL1);
    EUSART_Write(TX2dados.vH2);
    EUSART_Write(TX2dados.vL2);
...
void selecaoRX(){
    switch(ByteRx[0]){
        ...
        case 0x80:
            if(RXdados.vHx== 0x00){
            if(RXdados.vLx == 0x00){ swit = 0;}
            else if(RXdados.vLx == 0x01){ swit = 1;}
            }
            countRx = 0;
            break;
...
void desloca(){
    RXdados.vHx = ByteRx[1];
    RXdados.vLx = ByteRx[2];
    if(RXdados.vLx>0x7F){return;}
    if(RXdados.vHx>0x7F){return;}
    RXdados.vLx = RXdados.vLx << 1;
    RXdados.Rtx = (RXdados.Rtx >> 1);
    RXdados.vHx = RXdados.vHx & 0x7F;
    selecaoRX();
}
